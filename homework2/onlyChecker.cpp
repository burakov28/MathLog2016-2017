...
bool isModusPonens(expression * expr) {
...
bool isGeneralization(expression * expr) {
    if (expr -> name != "->") return false;
    expression * left = expr -> args[0];
...
bool isReduction(expression * expr) {
    if (expr -> name != "->") return false;
